...
class SyncEmpHandler extends AbstractHandler {
    async handleRequest(input, context) {
        ...
        const fileName = `syncemprequest${Math.random()}.json`;
        console.log(`Request File Name: ${DBUtils.S3BUCKET_BULKEMP}/${fileName}`);
        await S3bucketUtils.uploadText(DBUtils.S3BUCKET_BULKEMP, fileName, this.jsonQueryString);
...
exports.handler = async (event, context) => {
    const handler = new SyncEmpHandler();
